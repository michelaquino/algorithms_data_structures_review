...
func main() {
	...
	for _, t := range testCases {
		...
		// fmt.Println(substring("leetcode", 4, 8))
		// return

		fmt.Println("=========================================")
		output := wordBreak(t.s, t.wordDict)
...
func wordBreak(str string, wordDict []string) bool {
	...
	wordFoundEndIn := make([]bool, len(str)+1)
	wordFoundEndIn[0] = true

	for i := 1; i <= len(str); i++ {
		for j := 0; j < i; j++ {
			substring := str[j:i]
			if wordFoundEndIn[j] && wordDictMap[substring] {
				wordFoundEndIn[i] = true
				break
			}
		}
	}
	return wordFoundEndIn[len(str)]
}
